# -*- coding: utf-8 -*-
"""
Created on Wed Apr  3 00:23:14 2024
...
from common.intrinsic_encoding_decoding import intrinsic_parameters_encoding, intrinsic_parameters_decoding
...
def step(split, opt, actions, dataloader, model_pos, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(t):
        ...
        if split == 'train':
            ...
            #---------------Model----------------------------------------------
            p3d_pred = model_pos(p2d_cpn)  # [b t j c n]

        else:
            p2d_gt, p2d_cpn, cam, p3d_gt, traj_gt, p3d_pred = input_augmentation(p2d_gt, p2d_cpn, cam, p3d_gt, traj_gt, model_pos)
        ...
        bl_pred_0 = get_bl_from_pos(p3d_pred[...,0])              # [b t 10]
        bl_pred_1 = get_bl_from_pos(p3d_pred[...,1])              # [b t 10]
        bl_pred_2 = get_bl_from_pos(p3d_pred[...,2])              # [b t 10]
        bl_pred_3 = get_bl_from_pos(p3d_pred[...,3])              # [b t 10]
        
        bl_pred = torch.stack([bl_pred_0, bl_pred_1, bl_pred_2, bl_pred_3], dim=-1)
        factor = bl_pred[...,1:2,:].unsqueeze(-2)
        p3d_pred = p3d_pred / factor
        
        
        p3d_abs_gt = p3d_gt + traj_gt
        p2d_pro = p3d_abs_gt[...,0:2,:] / p3d_abs_gt[...,2:3,:]
                        
        # loss 
        loss_p2d_cpn = mpjpe_cal(p2d_gt, p2d_pro)
        ...
        # loss prompting
        t.set_description('2d_cpn({0:,.4f}), 3d_pred({1:,.4f})'.format(loss_all["loss_p2d_cpn"].avg, loss_all["loss_p3d_pred"].avg))
    ...
    if split == 'train':
        ...
        return mpjpe_2d_cpn, mpjpe_3d_pred
...
def input_augmentation(p2d_gt, p2d_cpn, cameras, p3d_gt, traj_gt, model_pos):
    ...
    #---------------Model----------------------------------------------
    p3d_pred_non_flip = model_pos(p2d_cpn_non_flip)  # [b t j c n]
    
    return p2d_gt_non_flip, p2d_cpn_non_flip, cameras_non_flip, p3d_gt_non_flip, traj_gt_non_flip, p3d_pred_non_flip
...
if __name__ == '__main__':
    ...
    model = {}
    model['pos_train'] = SVJFormer(A_s, A_v, num_frames=1, num_joints=17, num_views=4, in_chans=2, out_chans=3, embed_dim_ratio=128, depth=6,
            num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None, drop_path_rate=0.1).cuda()
    model['pos_test'] = SVJFormer(A_s, A_v, num_frames=1, num_joints=17, num_views=4, in_chans=2, out_chans=3, embed_dim_ratio=128, depth=6,
    ...
    for epoch in range(1, opt.nepoch):
        ...
        mpjpe_p2d_cpn, mpjpe_p3d_pred = val(opt, actions, test_dataloader, model)
        
        data_threshold = mpjpe_p3d_pred 
        ...
        if not opt.train:
            ...
            logging.info('epoch: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred))
            logging.info('epoch: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred))
            print('e: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred))
            print('e: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred))
