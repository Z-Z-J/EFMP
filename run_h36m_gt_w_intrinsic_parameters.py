...
def step(split, opt, actions, dataloader, model_pos, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(t):
        ...
        if split == 'train':
            ...
            #---------------Model----------------------------------------------
            p3d_pred = model_pos(p2d_gt)  # [b t j c n]
        else:
            p2d_gt, p2d_cpn, cam, p3d_gt, traj_gt, p3d_pred = input_augmentation(p2d_gt, p2d_cpn, cam, p3d_gt, traj_gt, model_pos)
     
        
        p3d_abs_pred_s, R_pred, T_pred, loss_con = BPMA(p2d_gt, p3d_pred, iter_step=2)
        ...
        # loss prompting
        t.set_description('2d_cpn({0:,.4f}), 3d_pred({1:,.4f}), con({2:,.4f}), 3d_s_pred({3:,.4f}), traj_s_pred({4:,.4f})'.format(loss_all["loss_p2d_cpn"].avg, 
    ...
    if split == 'train':
        ...
        return mpjpe_2d_cpn, mpjpe_3d_pred, mpjpe_3d_s_pred, mpjpe_traj_s_pred
...
def input_augmentation(p2d_gt, p2d_cpn, cameras, p3d_gt, traj_gt, model_pos):
    ...
    #---------------Model----------------------------------------------
    p3d_pred_non_flip = model_pos(p2d_gt_non_flip)  # [b t j c n]
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.nepoch):
        ...
        mpjpe_p2d_cpn, mpjpe_p3d_pred, mpjpe_p3d_pred_s, mpjpe_traj_pred_s = val(opt, actions, test_dataloader, model)
        ...
        if not opt.train:
            ...
            logging.info('epoch: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f, p3d_pred_s: %.2f, traj_pred_s: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred, mpjpe_p3d_pred_s, mpjpe_traj_pred_s))
            logging.info('epoch: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f, p3d_pred_s: %.2f, traj_pred_s: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred, mpjpe_p3d_pred_s, mpjpe_traj_pred_s))
            print('e: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f, p3d_pred_s: %.2f, traj_pred_s: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred, mpjpe_p3d_pred_s, mpjpe_traj_pred_s))
            print('e: %d, lr: %.7f, p2d_cpn: %.2f, p3d_pred: %.2f, p3d_pred_s: %.2f, traj_pred_s: %.2f' % (epoch, lr, mpjpe_p2d_cpn, mpjpe_p3d_pred, mpjpe_p3d_pred_s, mpjpe_traj_pred_s))
